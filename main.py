...
def train(args, params):
    # Model
    model = nn.center_net_18(len(params['names'].values()))
    model.cuda()
    ...
    filenames = []
    for filename in os.listdir('../Dataset/COCO/images/train2017/'):
    ...
    criterion = util.ComputeLoss(args.input_size, len(params['names'].values()))
...
def test(args, model=None):
    filenames = []
    for filename in os.listdir('../Dataset/COCO/images/val2017/'):
    ...
    for samples, targets in p_bar:
        ...
        outputs = decoder(*outputs, input_size=args.input_size)
